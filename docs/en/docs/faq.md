...
??? question "Why configs connect using v2rayNG and not Streisand for example?"
    ZAGROOO tries adapting new cores' features ASAP, meanwhile some developers just upgrade cores to the latest version regardless of full features adaption and optimizations. So you should communicate such issues with their own developers.
...
??? question "Why do I get an error when I go to the `panel/` address?"
    You can only install ZAGROOO using ZAGROOO Wizard and save panel address immediately. If you lost it install another one.  
...
??? question "Why v2rayN cannot ping test configs?"
    Right now v2rayN is experiencing some issues with custom configs and ZAGROOO panel configs are all custom. No worries, just enable config and use it. You also have Best Ping config in all subscriptions which connects to best IP automatically, so you don't need to test all configs everytime.

??? question "Why sing-box throws error while importing subscription?"
    ZAGROOO only supports sing-box 1.12.0 or higher
