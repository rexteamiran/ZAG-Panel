...
export async function setupTelegramWebhook(request: Request, env: Env): Promise<Response> {
    ...
    try {
        ...
        await storePut(env, 'telegramBot', bot);
...
export async function removeTelegramBot(request: Request, env: Env) {
    ...
    try {
        ...
        const { telegramBotToken } = await storeGet<TelegramBot>(env, 'telegramBot') ?? { telegramBotToken: '' } as TelegramBot;
...
export async function handleTelegramWebhook(request: Request, env: Env): Promise<Response> {
    const tgBot = await storeGet<TelegramBot>(env, 'telegramBot');
...
/* ==========================================================================
   ...
   Fired from the request path (Workers have no background timers), so they
   piggyback on whatever traffic the panel is already serving. Each alert is
   latched in PanelLimits.alertState so it is sent once per quota cycle.
   ========================================================================== */

const QUOTA_ALERT_THRESHOLD = 0.8;
const EXPIRY_ALERT_DAYS = 3;

async function pushAlert(env: Env, text: string): Promise<void> {
    const bot = await storeGet<TelegramBot>(env, 'telegramBot');
...
/**
 * Checks quota and expiry thresholds and sends whichever alerts are newly due.
 * Returns updated limits when an alert latch changed, otherwise null.
 */
export async function checkAlerts(
    env: Env,
    limits: PanelLimits,
    usage: UsageSnapshot
): Promise<PanelLimits | null> {
    const state = { ...limits.alertState };
    const messages: string[] = [];

    if (limits.alertQuota && limits.limitTotalBytes) {
        const ratio = usage.totalBytes / limits.limitTotalBytes;

        if (ratio >= 1 && !state.quota100) {
            state.quota100 = true;
            state.quota80 = true;
            messages.push([
                `🛑 <b>${_project_} volume exhausted</b>`,
                '',
                `<b>Used:</b> ${formatBytes(usage.totalBytes)} / ${formatBytes(limits.limitTotalBytes)}`
            ].join('\n'));
        } else if (ratio >= QUOTA_ALERT_THRESHOLD && !state.quota80) {
            state.quota80 = true;
            messages.push([
                `📊 <b>${_project_} volume at ${Math.round(ratio * 100)}%</b>`,
                '',
                `<b>Used:</b> ${formatBytes(usage.totalBytes)} / ${formatBytes(limits.limitTotalBytes)}`,
                `<b>Remaining:</b> ${formatBytes(Math.max(0, limits.limitTotalBytes - usage.totalBytes))}`
            ].join('\n'));
        }
    }

    if (limits.alertExpiry && limits.expireAt && !state.expirySoon) {
        const daysLeft = Math.ceil((limits.expireAt - Date.now()) / 86_400_000);

        if (daysLeft <= EXPIRY_ALERT_DAYS && daysLeft >= 0) {
            state.expirySoon = true;
            messages.push([
                `📅 <b>${_project_} subscription ending</b>`,
                '',
                `<b>Days left:</b> ${daysLeft}`,
                `<b>Expires:</b> ${new Date(limits.expireAt).toISOString().split('T')[0]}`
            ].join('\n'));
        }
    }
    ...
    for (const message of messages) {
        await pushAlert(env, message);
    }

    return { ...limits, alertState: state };
}
