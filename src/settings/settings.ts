...
Object.assign(globalThis, {
    ...
    _V2_: atob('djJyYXk='),
    _project_: atob('WkFHUk9PTw=='),
    _project_SM_: atob('emFncm9vbw=='),
    _repo_: atob('aHR0cHM6Ly9naXRodWIuY29tL3JleHRlYW1pcmFuL1pBRy1QYW5lbA=='),
    _wizard_repo_: atob('aHR0cHM6Ly9naXRodWIuY29tL3JleHRlYW1pcmFuL1pBRy1XaXphcmQ='),
    _website_: atob('aHR0cHM6Ly9yZXh0ZWFtaXJhbi5naXRodWIuaW8vWkFHLVBhbmVsLw=='),
...
export function init(request: Request, env: Env) {
    if(env.UUID || env.TR_PASS || typeof EMBEDED_SETTINGS === 'undefined') {
        throw new Error(`${_project_} Panel can only be installed using the <a href="${_wizard_repo_}" target="_blank">${_project_} Wizard</a>.`);
    }
