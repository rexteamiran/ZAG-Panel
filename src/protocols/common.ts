...
import { getActiveLimits, throttle } from '@limits';
import { recordBytes, scheduleFlush } from '@usage';
...
async function remoteSocketToWS(
    ...
    const writableStream = new WritableStream({
        ...
        async write(chunk, controller) {
            ...
            recordBytes('down', size);
            await throttle('down', size, getActiveLimits()?.downSpeedKbps ?? 0);
    ...
    const stream = new ReadableStream({
        start(controller) {
            webSocketServer.addEventListener('message', (event) => {
                ...
                recordBytes('up', event.data?.byteLength ?? event.data?.size ?? 0);
                controller.enqueue(event.data);
            });

            webSocketServer.addEventListener('close', () => {
                safeCloseWebSocket(webSocketServer);
                // Persist whatever this connection accumulated; an isolate can
                // be torn down right after the socket closes.
                scheduleFlush(true);
                if (readableStreamCancel) return;
                controller.close();
            });
